...
function session(options){
  ...
  store.generate = function(req){
    if(!req.sessionID) {
      ...
      var isGuest = getRawCookie(req, 'guest');
      if(!isGuest || isGuest == '') {
        var createTime = parseInt(req.sessionID.substr(req.sessionID.lastIndexOf('-') + 1));
        var maxAge = cookie.maxAge || Number.POSITIVE_INFINITY;
        if(!createTime || (Date.now() - createTime) >= maxAge) {
          req.sessionID = generateId(req);
        }
      }
    }
  ...
  var sessionMid = function(req, res, next) {
    // self-awareness
    ...
    onHeaders(res, function(){
      ...
      if (!shouldSetCookie(req)) {
        //console.log('shouldSetCookie: ' + false);
        return;
      }
  ...
  return {
    ...
    session: sessionMid,
    getcookie: getcookie,
    setcookie: function (req, res, name, value) {
      var secrets = secret || [req.secret];
      setcookie(res, name, value, secrets[0], req.session.cookie.data);
    }
  }
};
...
function generateSessionId(sess) {
  return uid(24) + '-' + new Date().getTime();
}
